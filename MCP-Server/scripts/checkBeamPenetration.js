/*
 * ⚠️ DEV-ONLY SCAFFOLDING — NOT PART OF THE SANCTIONED MCP PATH ⚠️
 * ---------------------------------------------------------------------------
 * 收編自鈺傑 (SEven777-a) rc 分支。此腳本直連 raw WebSocket (ws://localhost:8964)
 * 並自行拼裝 advanced_analyze / visualize_penetration 封包，繞過 MCP-Server 的
 * tool 定義與 Revit command dispatcher，違反 CLAUDE.md 的「Do Not Bypass MCP」守則。
 *
 * 保留於本 feature 分支僅作為開發期驗證/回歸腳手架。
 * 在併入 main 之前，必須改造為正規的 MCP 工具編排
 * （scan_penetrated_beams_in_view → analyze_beam_penetration → 規則檢核 → visualize_penetration），
 * 對應 skill: .claude/skills/check-beam-penetration/SKILL.md。
 * ---------------------------------------------------------------------------
 */
