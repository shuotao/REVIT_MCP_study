...
const PARAM_NAMES = ["s_CW_?≤ÁÅ´?≤Á??ßËÉΩ"];
...
function getColor(val) {
    ...
    if (val.includes("2") && val.includes("Â∞èÊ?")) return COLOR_MAP["2HR"];
    if (val.includes("1") && val.includes("Â∞èÊ?")) return COLOR_MAP["1HR"];
    if (val.includes("??) || val === "0" || val === "-") return COLOR_MAP.NONE;
...
ws.on('open', () => {
    ...
    output('Using parameter: s_CW_?≤ÁÅ´?≤Á??ßËÉΩ');
...
ws.on('message', (data) => {
    ...
    if (stage === 'get_view') {
        ...
        if (res.Data.Parameters) {
            const p = res.Data.Parameters.find(x => x.Name === "s_CW_?≤ÁÅ´?≤Á??ßËÉΩ");
