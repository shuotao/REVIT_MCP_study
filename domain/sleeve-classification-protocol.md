...
## 5. 穿梁套管法規檢核規則 (Beam Penetration Inspection Rules)

為確保結構安全，穿梁套管的法規檢核必須依循以下幾何定位檢核規則：

### 5.1 水平淨距離檢核 (Horizontal Clearance)
* **檢核定義**：套管的**外孔邊緣**到梁端（柱邊）的淨距離 $d$（計算式為：中心投影距離 - 套管半徑 $D/2$），必須符合以下限制。**（嚴禁以套管中心線計算）**
* **大梁檢核規則**：
  * **禁開區 ($d < 1.0H$)**：距梁端（柱邊）不足 $1.0H$ 區域禁止開孔。
  * **限制區 ($1.0H \le d < 1.5H$)**：最大允許孔徑 $D \le H/4$。
  * **一般區 ($d \ge 1.5H$)**：最大允許孔徑 $D \le H/3$。
* **小梁檢核規則**：
  * **禁開區 ($d < 0.5H$)**：距梁端不足 $0.5H$ 區域禁止開孔。
  * **一般區 ($d \ge 0.5H$)**：最大允許孔徑 $D \le H/3$。

### 5.2 垂直淨距離檢核 (Vertical Clearance)
...
### 5.3 相鄰套管淨距離檢核 (Sleeve Spacing Clearance)
* **檢核定義**：同一支梁上相鄰兩套管之**外邊緣到外邊緣的淨距離** $S_{net}$（計算式為：兩套管中心距離 - 半徑 $D_1/2$ - 半徑 $D_2/2$），必須符合以下限制：
  * $S_{net} \ge (D_1 + D_2)$
  * 其中 $D_1$ 與 $D_2$ 分別為該相鄰兩套管之直徑。亦即，**相鄰套管邊到邊的淨距離，必須大於或等於兩孔徑之和**。
