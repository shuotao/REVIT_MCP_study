...
3.  **依賴關係**：`MCP-Server` 透過 WebSocket (Port 11111) 與 Revit Add-in 通訊。
4.  **代碼品質**：
...
- **座標轉換**：
    - 取得元素的 `BoundingBox`。
    - 標註位置線應定義在 `(max + min) / 2` 的 center 軌跡上，以確保標註文字不與邊界牆重疊。
...
### [L-006] Revit MCP 通訊埠號統一設定 (2026-03-16)
- **規則**：不要再連 8964 了，從現在起請一律改用 `11111` 埠號通訊。
- **實踐**：
    - C# 預設與 config.json 皆應設為 `11111`。
    - Node.js (`socket.ts`) 客戶端同樣預設請求 `11111`。
    - 用戶端 `settings.json` 的 `REVIT_PORT` 也要對應調整。
