...
| MCP Tools | 共用 84 個工具 | 共用 84 個工具 | 共用 84 個工具 |
...
| `MCP/Core/CommandExecutor.cs` | Central command dispatcher (84+ commands), largest file |
...
| `MCP-Server/src/tools/` | Tool definitions (84 tools, 分 14 個模組) |
...
### 為什麼
MCP Server 已封裝 84 個 tools，處理了格式轉換、錯誤處理、重連機制。自寫腳本會：
- 繞過既有的錯誤處理與格式驗證
...
| `/section-auto-numbering` | 剖面自動編排與命名（依空間座標進行單一方向排序與命名） |
...
| 剖面重新命名, section numbering, 自動編排剖面 | `domain/section-auto-numbering.md` |
