...
| MCP Tools | 共用 78 個工具 | 共用 78 個工具 | 共用 78 個工具 |
...
| `MCP/Core/CommandExecutor.cs` | Central command dispatcher (78+ commands), largest file |
...
| `MCP-Server/src/tools/` | Tool definitions (78 tools, 分 16 個模組) |
...
### Deferred Tools 載入規則（每個 Session 必做）

本專案的 `mcp__revit-mcp__*` 工具是 **deferred tools** — session 開始時只有名字，schema 未載入。
在呼叫任何 `revit-mcp` 工具之前，**必須先用 `ToolSearch` 載入 schema**：

```
ToolSearch select:mcp__revit-mcp__import_excel_to_drafting_views
```

- 用 `select:` 語法精確載入需要的工具，不要用模糊 keyword
- 若 `ToolSearch` 找不到 `mcp__revit-mcp__*`，才判定 MCP Server 沒連上
- **DO NOT** 因為工具不在初始列表就回報「MCP 沒連上」— 先 `ToolSearch`，再下結論
...
| `/view-category-visibility` | 視圖類別可見性控制（隱藏/顯示類別，含連結模型） |
...
| `/floor-plan-from-template` | 以範本 FloorPlan 批次建立樓層平面圖（複製 Type/Template/Phase），可選配 `align_view_cropbox_to_element` 對齊到圖框 Detail Group |
| `/sheet-management` | 圖紙與視圖埠管理（批次建立、重新排序） |
| `/copy-sheets-cross-project` | 跨專案複製含 model views 的圖紙（Revit 原生 Insert from File 不支援）。兩階段：read_source_file_sheets 偵測衝突 → 使用者確認 → copy_sheets_from_file 執行（含 sheet custom parameters 如圖集/Discipline 自動複製）。view 四級分類（Tier 1 自動建 / Tier 2 部分建 / Tier 3 只匹配 / Tier 4 人工）。內含 sync_sheet_parameters_from_source 補丁工具修補舊 sheet metadata。Pre-flight checklist + socket timeout 處理 SOP 完整 |
| `/align-views-on-sheets` | 批次對齊 view 範圍（套 ScopeBox）+ titleblock instance 位置 + viewport 在 sheet 上的位置（依錨點+offset）。三階段：set_scope_box_for_views 統一範圍 → align_titleblocks_on_sheets 對齊圖框 instance（可選，處理 change_element_type 後 instance origin 不齊）→ position_viewports_on_sheet 統一位置（強制 dryRun 預覽 SOP）。常接在 copy-sheets-cross-project 之後 |
| `/batch-apply-view-template` | 批次將指定 ViewTemplate 套用到多個 view（`batch_apply_view_template`：以 sheets/viewIds/viewNames 選取，可加 viewTypeFilter，強制 dryRun 預覽 SOP，預設跳過已使用相同 template 的 view）。常接在 copy-sheets-cross-project 之後統一視圖樣式 |
| `/stair-hidden-line` | 剖面隱藏樓梯可視化（虛線詳圖線） |
| `/claude-md-sync` | CLAUDE.md 雙向同步驗證（合併/Skill異動/Tools異動後觸發） |
| `/excel-to-legend` | Excel 表格批次匯入 Revit Drafting View（一站式 `import_excel_to_drafting_views`：列印範圍、合併儲存格、長文字 wrap、覆蓋時自動還原 viewport 位置） |
| `/scale-drafting-width` | DraftingView 表格縮放（寬度 X 軸 + 行高 Y 軸，獨立錨點，逐 View 獨立 Transaction） |
| `/text-note-batch` | TextNote 批次操作（跨 DraftingView 依文字內容搜尋並平移） |
| `/copy-detail-items` | 批次複製詳圖項目（DetailCurve/TextNote/FilledRegion/DetailComponent/Dimension）從一個來源視圖到多個目標視圖（同專案內），支援類別過濾、指定元素、XY 偏移、dryRun 預覽 |
| `/batch-material` | 批次材質修改（查詢類型→使用者確認→批次替換，影響 3D 渲染） |
| `/batch-room-height` | 批次修改房間上限高度（依名稱/用途分組→使用者確認→一次套用 Upper Limit + Limit Offset，不動樓層） |
| `/unjoin-geometry` | 批次解除 Revit 幾何接合（通用版 `unjoin_element_joins`，以任一類別為中心解除其與 8 類預設 target 的接合，含「跑兩次驗證」SOP；session-scoped 還原） |
| `/dedup-detail-elements` | 找出並刪除視圖中重複的 detail element（同 Type + 位置量化後相同），優先保留 Detail Group 內副本、刪除 group 外副本（`dedup_detail_elements_in_view`，強制 dryRun → 確認 → 執行 SOP，三類狀態分流：to_dedup 推薦處理 / all_in_groups 保守不動 / ambiguous_no_group 由使用者決定） |
...
## Domain Knowledge & Workflow Files（34 個）
...
| 房間高度, 房高, upper limit, limit offset, room height, ceiling height | `domain/room-height-limit.md` |
| 解除接合, 取消結合, unjoin, unjoin geometry, 白模準備, render preparation | `domain/unjoin-geometry-workflow.md` |
| 重複, dedup, deduplicate, duplicate detail, 清理視圖, 重複的標頭/尺寸/線/填充 | `domain/dedup-detail-elements-workflow.md` |
