...
| `/align-views-on-sheets` | 批次對齊 view 範圍（套 ScopeBox）+ viewport 在 sheet 上的位置（依錨點+offset）。兩階段：set_scope_box_for_views 統一範圍 → position_viewports_on_sheet 統一位置（強制 dryRun 預覽 SOP）。常接在 copy-sheets-cross-project 之後 |
