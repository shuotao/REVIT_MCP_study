...
| MCP Tools | 共用 92 個工具 | 共用 92 個工具 | 共用 92 個工具 |
...
| `MCP/Core/CommandExecutor.cs` | Central command dispatcher (92+ commands), largest file |
...
| `MCP-Server/src/tools/` | Tool definitions (92 tools, 分 15 個模組) |
...
### 為什麼
MCP Server 已封裝 92 個 tools，處理了格式轉換、錯誤處理、重連機制。自寫腳本會：
- 繞過既有的錯誤處理與格式驗證
...
|-----------------|------|
| 容積率, 防火構造, 走廊寬度, 樓梯寬度, 無障礙, 建築技術規則, building code, 法規條號 | `domain/references/building-code-tw.md` |
| 機械出圖, mechanical, assembly, 組件, BIP, 視圖生成, 比例計算 | `domain/mechanical-part-doc.md` |
| MEP, pyRevit, 機電, 擴展, 研究指南, MEP guide | `domain/mep-extension-guide.md` |
...
| 梁穿孔, 套管穿梁, beam penetration, sleeve through beam, 結構穿梁 | `domain/beam-penetration-base.md` |
| RC 梁穿孔, 混凝土梁, Zone A, Zone B, Zone C, 禁開區, 限制區, 許可區 | `domain/beam-penetration-rc.md` |
| SC 梁穿孔, 鋼梁, 腹板開孔, web opening, flange | `domain/beam-penetration-sc.md` |
| SRC 梁穿孔, 鋼骨混凝土, composite beam, internal steel | `domain/beam-penetration-src.md` |
