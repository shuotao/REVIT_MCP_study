...
<div class="container">
  ...
  <div class="hero-section">
    ...
    <p class="hero-sub">支援將多項勾選綠建材打包為「專案材料組合 (Material Set)」，可自訂 Set 名稱（如 A棟客房Set、外牆隔熱Set）並一鍵批次推送到 Revit 模型與共享參數。</p>
  </div>

  <div class="search-filter-bar">
    <div class="search-input-group">
      <input type="text" id="searchInput" class="search-input" value="塗料" placeholder="🔍 輸入任意關鍵字（如：塗料、水泥漆、乳膠漆、地板、矽酸鈣板、隔音、天花板...）" oninput="filterMaterials()">
    </div>
    <div class="quick-pills">
      <button class="pill-btn active" onclick="setSearch('塗料')">🎨 塗料漆類</button>
      ...
      <button class="pill-btn" onclick="setSearch('')">🌐 全部 {total_count} 項建材</button>
    </div>
  </div>
...
<script>
  ...
  function promptSaveSet() {{
    ...
    localStorage.setItem('tabc_material_sets', JSON.dumps(materialSets));
    updateSetCountBadge();
...
print(f"Successfully injected {total_count} master items & Set Management UI into assets/green-material-showcase.html!")
