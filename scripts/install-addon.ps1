...
# 複製 DLL
# 注意：RevitMCP.addin 的 <Assembly> 是相對路徑 "RevitMCP\RevitMCP.dll"（見 CLAUDE.md 部署規則、
# preflight-check.ps1 檢查點），DLL 必須放在 Addins\{版本}\RevitMCP\ 子資料夾，放根層 Revit 載不到。
...
# 只複製 Newtonsoft 會讓 quantity-takeoff-excel 等工具在執行期 FileNotFoundException。
