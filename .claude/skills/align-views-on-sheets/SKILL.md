...
description: "批次對齊 view 範圍 + viewport 在 sheet 上的位置：兩階段流程結合 ScopeBox 套用與 viewport 定位。階段一用 set_scope_box_for_views 把一組 view 的 CropBox 綁到指定 ScopeBox（範圍統一）；階段二用 position_viewports_on_sheet 把這些 viewport 移到 sheet 上的指定錨點位置（位置統一）。適用於跨專案複製後對齊新 view、批次統一系列圖紙的 layout、出圖前最後對齊。觸發條件：使用者提到對齊 view、統一範圍、套用 scope box、批次定位 viewport、view 對齊 title block、align views to scope box、position viewports on sheet、sheet layout 對齊、批次出圖整理。工具：set_scope_box_for_views、position_viewports_on_sheet、get_viewport_map、get_sheet_viewport_details。"
---
...
| **B. 統一位置** | 移動 viewport 到 sheet 上指定錨點 | 不同 sheet 上的 viewport 都對齊到 title block 的同一個位置 |

兩階段順序：**先 A 再 B**。先套 ScopeBox 會改變 viewport 在 sheet 上的尺寸（因為 view 範圍變了），所以位置定位必須在範圍定下來之後做。
